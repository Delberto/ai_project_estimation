...
class ProjectMetadata(BaseModel):
    ...
    Pensado para rellenarse (hoy a mano, mañana vía extracción LLM) y
    reinyectarse en prompts posteriores sin re-preguntar al usuario.
    """
...
class ConversationHistory:
    ...
    aparte y siempre se antepone en :meth:`to_messages`. Así, aunque
    descartemos turnos antiguos, las instrucciones del sistema permanecen.

    Un "turno" = un mensaje ``user`` (el ``assistant`` que le sigue, si
    existe, se elimina junto con él al hacer trim).
    """
    ...
    def __init__(self, *, system_prompt: str, max_turns: int = 10) -> None:
        ...
        # Solo user/assistant. El system va fuera para no perderlo al trim.
    ...
    def to_messages(self) -> list[Message]:
        """Lista lista para LiteLLM/OpenAI: ``[system, ...user/assistant]``."""
        return [
    ...
    def _trim(self) -> None:
        """Descarta los turnos más antiguos hasta caber en ``max_turns``.

        Al borrar, si el mensaje user va seguido de un assistant, se elimina
        el par completo para no dejar respuestas huérfanas en el historial.
        """
        while self.turn_count > self.max_turns:
            # Primer user = turno más viejo.
            drop_index = next(
                index
